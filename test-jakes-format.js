/**
 * Test Jake's LaTeX format PDF generation
 */
const documentGenerationService = require('./src/services/documentGeneration.service');
const fs = require('fs').promises;

async function testJakesFormat() {
  console.log('Testing Jake\'s LaTeX format PDF generation...\n');
  
  // Jake's exact resume content from the LaTeX file
  const testData = {
    resumeText: `Jake Ryan
...
Libraries: pandas, NumPy, Matplotlib`,
    ...
    improvements: ["Matched Jake's format exactly"],
    changes: {}
  };
  
  try {
    // Test PDF
    console.log('Generating Jake\'s format PDF...');
    ...
    await fs.writeFile('./test-jakes.pdf', pdfBuffer);
    console.log('✅ PDF saved to: ./test-jakes.pdf');
    
    console.log('\nTest complete! Open test-jakes.pdf to verify it matches Jake\'s LaTeX format');
