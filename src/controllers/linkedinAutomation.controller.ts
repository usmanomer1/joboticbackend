...
export class LinkedInAutomationController {
  ...
  async startAutomation(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { userId, config } = validation.data;
      ...
      // Log API call
      await this.logApiCall(userId, 'start_automation', { config });
      ...
      const jobSearchConfig: JobSearchConfig = {
        ...
        easyApplyOnly: config.filters?.easyApplyOnly ?? true,
  ...
  async getStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Get progress
      const progress = await this.automationService.getProgress(sessionId);

      // Check for active interventions
      const logs = await this.supabaseService.getSessionLogs(sessionId, 10);
      const recentIntervention = logs.find(
        log => log.event_type === 'intervention_detected' && 
        new Date(log.created_at).getTime() > Date.now() - 5 * 60 * 1000 // Last 5 minutes
      );

      // Map internal status to Browser Use format
      const statusMap: Record<SessionStatus, string> = {
        [SessionStatus.PENDING]: 'pending',
        [SessionStatus.RUNNING]: 'running',
        [SessionStatus.PAUSED]: 'paused',
        [SessionStatus.INTERVENTION_REQUIRED]: 'intervention_required',
      ...
      if (progress.processedJobs > 0) {
        currentStep = `processing_jobs_${progress.currentPage}`;
      }
      ...
      // Build response
      const response: any = {
        status: statusMap[session.status as SessionStatus] || 'unknown',
        progress: {
      ...
      if (recentIntervention && session.status === SessionStatus.INTERVENTION_REQUIRED) {
        const interventionDetails = recentIntervention.details as any;
        response.interventionRequired = {
          type: interventionDetails.intervention_type,
          message: interventionDetails.message,
          debugUrl: interventionDetails.debug_url
        };
        response.liveViewUrl = interventionDetails.debug_url;
      }

      // Add live view URL if session is active
      if (session.status === SessionStatus.RUNNING && session.browserbase_session_id) {
        const browserbaseManager = new BrowserbaseSessionManager();
        response.liveViewUrl = await browserbaseManager.getDebugUrl(session.browserbase_session_id);
      }
  ...
  async pauseAutomation(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Pause the session
      await this.automationService.pauseSession(sessionId);
  ...
  private setupEventListeners(): void {
    ...
    this.automationService.on(AutomationEventType.INTERVENTION_REQUIRED, (data) => {
      ...
      interventionStates.set(sessionId, intervention);
    });
